...
async function generateReview(sourceText: string, apiKey: string, model: string): Promise<GeneratedReviewFields> {
  ...
  return {
    ...
    seo_title: String(parsed.seo_title ?? "").slice(0, 80),
    seo_description: String(parsed.seo_description ?? "").slice(0, 170),
    seo_intro: String(parsed.seo_intro ?? "").slice(0, 500),
    review_body: String(parsed.review_body ?? ""),
    key_ingredients_structured: Array.isArray(parsed.key_ingredients_structured) ? parsed.key_ingredients_structured.slice(0, 8) : [],
    skin_types: Array.isArray(parsed.skin_types) ? parsed.skin_types.slice(0, 5) : [],
    skin_concerns: Array.isArray(parsed.skin_concerns) ? parsed.skin_concerns.slice(0, 8) : [],
    faq: Array.isArray(parsed.faq) ? parsed.faq.slice(0, 10) : [],
    related_reviews: Array.isArray(parsed.related_reviews) ? parsed.related_reviews.slice(0, 6) : [],
    related_knowledge_articles: Array.isArray(parsed.related_knowledge_articles) ? parsed.related_knowledge_articles.slice(0, 6) : [],
