 ...
 *
 * Migrated from api/briefings-sync.ts (Vercel Cron) to this Supabase Edge
 * Function on 2026-09-22 -- see supabase/functions/product-review-sync/
 * index.ts's header comment for the full reasoning (Vercel's "sensitive"
 * env vars are genuinely unreadable, and this environment has no
 * authenticated Vercel CLI session to run `vercel crons run`, whereas it
 ...
 * cannot be set from this codebase/session; every invocation fails fast
 * with a clear "not configured" error until a human adds these):
 ...
 *
 * Manual verification override: ?force=true (still requires the same auth)
 * bypasses today's editorial cap and limits the run to a single article.
 *
 * Every Gemini call attempt is logged to pipeline_model_calls. A channel
 * candidate whose entire fallback chain is exhausted is queued in
 * pipeline_retry_queue for a lazy retry on this pipeline's next invocation.
 */
...
const FIRECRAWL_DAILY_LIMIT = Number(Deno.env.get("FIRECRAWL_BRIEFINGS_DAILY_LIMIT")) || 30;
const GEMINI_DAILY_LIMIT = Number(Deno.env.get("GEMINI_BRIEFINGS_DAILY_LIMIT")) || 100;
const GEMINI_PER_MINUTE_LIMIT = Number(Deno.env.get("GEMINI_BRIEFINGS_PER_MINUTE_LIMIT")) || 10;
...
async function recordApiUsage(admin: SupabaseAdmin, provider: "briefings-fc" | "briefings-gemini", purpose: string, success: boolean) {
  try {
...
async function withinDailyQuota(admin: SupabaseAdmin, provider: "briefings-fc" | "briefings-gemini", limit: number): Promise<boolean> {
...
async function withinPerMinuteQuota(admin: SupabaseAdmin, provider: "briefings-fc" | "briefings-gemini", limit: number): Promise<boolean> {
...
async function searchPexels(query: string, exclude: Set<number>): Promise<StockPhoto | null> {
  const key = Deno.env.get("PEXELS_API_KEY_BRIEFINGS");
  if (!key) return null;
  ...
  const data = (await res.json().catch(() => null)) as { photos?: PexelsPhoto[] } | null;
...
async function searchUnsplash(query: string, exclude: Set<string>): Promise<StockPhoto | null> {
  const key = Deno.env.get("UNSPLASH_API_KEY_BRIEFINGS");
  if (!key) return null;
  ...
  const res = await fetch(url.toString(), { headers: { Authorization: `Client-ID ${key}`, "Accept-Version": "v1" } });
  if (!res.ok) return null;

  const data = (await res.json().catch(() => null)) as { results?: UnsplashResult[] } | null;
...
async function fetchCoverPhoto(imageQueries: string[], usedPexelsIds: Set<number>, usedUnsplashIds: Set<string>): Promise<StockPhoto | null> {
...
async function firecrawlSearchChannel(channel: SourceChannel, apiKey: string, limit: number): Promise<FirecrawlPage[]> {
  ...
  const payload = (await res.json().catch(() => null)) as { data?: FirecrawlSearchRow[] | { web?: FirecrawlSearchRow[] } } | null;
  if (!res.ok) return [];

  const rows: FirecrawlSearchRow[] = Array.isArray(payload?.data)
    ? (payload.data as FirecrawlSearchRow[])
    : Array.isArray((payload?.data as { web?: FirecrawlSearchRow[] } | undefined)?.web)
      ? (payload?.data as { web: FirecrawlSearchRow[] }).web
      : [];

  return rows
    .filter((r) => typeof r.url === "string" && typeof r.markdown === "string" && r.markdown.length > 400)
    .slice(0, 4)
    .map((r) => ({ url: r.url as string, title: r.title ?? (r.url as string), markdown: (r.markdown as string).slice(0, 12000) }));
}
...
async function researchChannel(admin: SupabaseAdmin, channel: SourceChannel, apiKey: string, hasRunBudget: boolean): Promise<ChannelResult> {
  ...
  if (!hasRunBudget) {
    return { channel, pages: [], madeRealCall: false, skippedReason: `Firecrawl run budget (${MAX_FIRECRAWL_CALLS_PER_RUN}) exhausted` };
  }
  if (!(await withinDailyQuota(admin, "briefings-fc", FIRECRAWL_DAILY_LIMIT))) {
    return { channel, pages: [], madeRealCall: false, skippedReason: `Firecrawl daily quota (${FIRECRAWL_DAILY_LIMIT}) reached` };
  }
...
function parseBriefingResponse(text: string): GeneratedBriefing {
  ...
  return {
    ...
    key_takeaways: Array.isArray(parsed.key_takeaways) ? parsed.key_takeaways.slice(0, 5) : [],
    ...
    reading_time_minutes: Math.max(5, Math.min(30, Number(parsed.reading_time_minutes) || 10)),
    image_queries: Array.isArray(parsed.image_queries) ? parsed.image_queries.slice(0, 3) : [],
  };
}
...
function weaveImages(body: string, images: InlineImage[]): { markdown: string; used: InlineImage[] } {
  if (images.length === 0) return { markdown: body, used: [] };

  const blocks = body.split(/\n{2,}/);
  const headingIndexes = blocks.map((b, i) => (b.trim().startsWith("## ") ? i : -1)).filter((i) => i > 1);

  const used: InlineImage[] = [];
  const slots = headingIndexes.filter((_, idx) => idx % 2 === 1).slice(0, images.length);

  slots.reverse().forEach((position, revIdx) => {
    const image = images[slots.length - 1 - revIdx];
    if (!image) return;
    used.push(image);
    const sourceLabel = image.creditUrl.includes("pexels.com") ? "Pexels" : "Unsplash";
    blocks.splice(position, 0, `![${image.alt}](${image.url})\n_Photo: ${image.creditName} on ${sourceLabel}_`);
  });
...
Deno.serve(async (req) => {
  ...
  const geminiKey = Deno.env.get("GEMINI_API_KEY_BRIEFINGS");
  const firecrawlKey = Deno.env.get("FIRECRAWL_API_KEY_BRIEFINGS");

  const missing = [!geminiKey && "GEMINI_API_KEY_BRIEFINGS", !firecrawlKey && "FIRECRAWL_API_KEY_BRIEFINGS"].filter(Boolean);
  if (missing.length > 0) {
    return new Response(
  ...
  const modelChain = [
    Deno.env.get("GEMINI_MODEL_BRIEFINGS") || "gemini-3.6-flash",
    Deno.env.get("GEMINI_MODEL_BRIEFINGS_FALLBACK_1") || "gemini-3.1-flash-lite",
  ...
  // Manual verification override: ?force=true (still requires the same auth
  // as every other invocation) bypasses today's editorial cap and limits
  ...
  try {
    ...
    if (!forceRun && (publishedToday ?? 0) >= DAILY_BRIEFINGS_CAP) {
      return jsonResponse({ ok: true, created: 0, message: "Daily briefings cap already met" });
    }
    ...
    for (const channel of SOURCE_CHANNELS) {
      ...
      try {
        ...
        if (result.skippedReason) errors.push(`${channel.id}: ${result.skippedReason}`);
    ...
    for (const candidate of [...retriedCandidates, ...freshCandidates]) {
      ...
      // ---- Gemini quota gate ----
      if (!(await withinDailyQuota(admin, "briefings-gemini", GEMINI_DAILY_LIMIT))) {
        errors.push(`Gemini daily quota (${GEMINI_DAILY_LIMIT}) reached -- stopping run`);
        break;
      }
      if (!(await withinPerMinuteQuota(admin, "briefings-gemini", GEMINI_PER_MINUTE_LIMIT))) {
        await sleep(15000);
        if (!(await withinPerMinuteQuota(admin, "briefings-gemini", GEMINI_PER_MINUTE_LIMIT))) {
      ...
      try {
        ...
        const imageQueries = briefing.image_queries.length > 0 ? briefing.image_queries : [channel.topicTag, "skincare routine", "skin close-up"];

        const usedPexelsIds = new Set<number>();
        const usedUnsplashIds = new Set<string>();
        const cover = await fetchCoverPhoto(imageQueries, usedPexelsIds, usedUnsplashIds);
        ...
        for (const iq of imageQueries.slice(1, 3)) {
          ...
          if (p) {
            inlinePhotos.push({ url: p.url, alt: p.alt, creditName: p.creditName, creditUrl: p.creditUrl });
          } else {
            const u = await searchUnsplash(iq, usedUnsplashIds);
            if (u) inlinePhotos.push({ url: u.url, alt: u.alt, creditName: u.creditName, creditUrl: u.creditUrl });
          }
          await sleep(150);
        }

        const cleanTitle = stripMarkdownSyntax(briefing.title);
        const { markdown, used: usedInlineImages } = weaveImages(briefing.body_markdown, inlinePhotos);
        const finalWordCount = countWords(markdown);

        // Dedup slug
        let slug = slugify(cleanTitle) || `daily-skinny-${Date.now()}`;
        const { data: slugTaken } = await admin.from("news_articles").select("id").eq("slug", slug).maybeSingle();
        ...
        const { error } = await admin.from("news_articles").insert({
          ...
          key_takeaways: (briefing.key_takeaways ?? []).map(stripMarkdownSyntax).slice(0, 5),
          sa_context_tag: stripMarkdownSyntax(briefing.sa_context_tag).slice(0, 40) || channel.topicTag,
          ...
          seo_description: stripMarkdownSyntax(briefing.seo_description).slice(0, 160),
          json_ld: buildJsonLd({
            slug,
            title: cleanTitle,
        ...
        if (err instanceof GeminiFatalError) {
          ...
          // this needs a human to check the Google AI Studio config, not a retry.
    ...
    try {
      const geminiCallAttempts = created + errors.filter((e) => e.includes("Gemini")).length;
      await admin.from("news_sync_runs").insert({
        run_date: today,
        articles_created: created,
        firecrawl_calls: firecrawlCallsThisRun,
        ai_calls: geminiCallAttempts,
        status: errors.length && created === 0 ? "failed" : errors.length ? "partial" : "ok",
    ...
    return jsonResponse({ ok: true, created, target, modelUsage, errors });
  } catch (err) {
