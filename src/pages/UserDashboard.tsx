              ...
              <div className="grid md:grid-cols-3 gap-6 mb-10">
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">Skincare analyses</p>
                  </CardContent>
                </Card>
              </div>
              ...
              {preorders.length > 0 && (
                <Card className="mb-6">
                  <CardHeader>
                    ...
                    <CardDescription>Track your edible skincare pouches order</CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      {preorders.map((order) => (
                        <div key={order.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                          ...
                          <div className="text-right">
                            ...
                            <Badge variant={order.status === "complete" ? "default" : "secondary"} className="text-xs">
                              {order.status}
                            </Badge>
                          </div>
                        </div>
              ...
              {recommendations.length > 0 && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Your personalized recommendations history</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                        ...
                        <div key={rec.id} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                          <div>
                            ...
                            <div className="flex gap-1 mt-1">
                              {rec.concerns.slice(0, 2).map((c) => (
                                <Badge key={c} variant="secondary" className="text-xs">{c}</Badge>
                              ))}
                              {rec.concerns.length > 2 && <Badge variant="secondary" className="text-xs">+{rec.concerns.length - 2}</Badge>}
                            </div>
                          </div>
                          <div className="text-right">
