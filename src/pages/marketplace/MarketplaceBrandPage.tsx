  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={brand?.description ?? `Shop ${brand?.name ?? "this brand"} on OpenHaus, curated by SkinLabs®.`}
        />
      </Helmet>
        ...
        <div className="max-w-lg lg:max-w-6xl mx-auto px-4 lg:px-8 pt-4">
          ...
          <div className="rounded-3xl overflow-hidden bg-stone-900">
            {coverImage ? (
              ...
              <div className="h-40 lg:h-64 flex items-center justify-center text-white text-2xl font-black">
                {brand?.name}
              </div>
            )}
          </div>
            ...
            {brand?.values && brand.values.length > 0 && (
              <div className="hidden sm:flex flex-wrap gap-1.5 justify-end max-w-xs">
                {brand.values.map((v) => (
                  <span
                    key={v}
                    className="rounded-full border border-stone-200 px-2.5 py-1 text-[10px] font-medium text-stone-600"
                  >
                    {v}
                  </span>
                ))}
